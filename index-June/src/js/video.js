(function () {
    ...
    function bindEvent() {
        var timer = null;
        // 列表事件
        $('.list-ul').on('click', 'dt', function () {
            if ($(this).hasClass('list-open')) {
                $(this).removeClass('list-open').nextAll().slideUp();
            } else {
                $(this).addClass('list-open').nextAll().slideDown();
        ...
        // 关闭弹窗
        $('.share-mask .close-btn').on('click', function () {
            $('.share-mask').fadeOut();
        })
        $('.audition-mask .close-btn').on('click', function () {
            $('.audition-mask').fadeOut();
        })
        $('.rate-mask .close-btn').on('click', function () {
            $('.rate-mask').fadeOut();
        })
        // rate event
        $('.rate-btn').on('click', function () {
            $('.rate-mask').fadeIn();
            $('.rate-ul li').removeClass('star').text("");
        })
        $('.rate-ul').on('mouseenter', 'li', function () {
            $(this).nextAll().removeClass('star').end()
                .text($(this).index() + 1).siblings().text("").end()
                .addClass('star').prevAll().addClass('star')
        })
        //app
        $('.app-btn').on('mouseenter', function () {
            $('.app-ewm').fadeIn();
        })
    ...
    function videoController() {
        ...
        function bindDrag(dom, bar, range, cb) {
            ...
            dom.onmousedown = function (e) {
                rangeValue = parseInt($('.' + range).width()) - parseInt($(dom).width()) / 2
                var event = e || window.event;
